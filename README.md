...
A `render.yaml` is included for one-click deploy. The backend serves the built frontend from the same origin — single service, no separate frontend hosting needed.

1. Push this repo to GitHub
2. In Render, create a new **Web Service** and connect the repo (or use **Blueprint** to auto-import `render.yaml`)
3. Set the following environment variables in the Render dashboard:
   - `GOOGLE_CLIENT_ID`
   - `GOOGLE_CLIENT_SECRET`
   - `BACKEND_URL` — your Render service URL, e.g. `https://photobook.onrender.com`
   - `SESSION_SECRET` — auto-generated by `render.yaml` if using Blueprint
4. In Google Cloud Console, add your Render URL as an authorized redirect URI:
